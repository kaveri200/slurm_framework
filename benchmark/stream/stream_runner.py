...
def build(array_size):
    ...
    try:
        ...
        cmake_lists = os.path.join(STREAM_DIR, "CMakeLists.txt")
...
def print_iteration_table(all_results):
    ...
    for i, res in enumerate(all_results, 1):
        ...
        print(f"|  {i:<3} | {copy:>10.2f} | {scale:>10.2f} | {add:>10.2f} | {triad:>10.2f} |")
    print("+------+------------+------------+------------+------------+")
...
def print_summary(threads, iterations, avg, status):
    ...
    print("|         |            |   (MB/s)   |           |")
    print("+---------+------------+------------+-----------+")
    print(f"| {threads:>7} | {iterations:>10} | {avg:>10.2f} | {run_status} |")
    print("+---------+------------+------------+-----------+")
...
def run():
    try:
        ...
        for i in range(iterations):
            ...
            if result.returncode != 0:
                print(result.stderr)
                raise Exception(f"stream failed on iteration {i+1}")
        ...
        print()
        avg_triad = sum(r.get("Triad", 0.0) for r in all_results) / len(all_results)
        print_summary(threads, iterations, avg_triad, 1)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python stream_runner.py [build|run]")
        sys.exit(1)
    ...
    if sys.argv[1] == "build":
        with open(os.path.join(BASE_DIR, "run_recipe.yaml")) as f:
            config = yaml.safe_load(f)
        user = config["user_inputs"]
        build(array_size=int(user.get("array_size", 10000000)))
